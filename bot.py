...
async def text_handler(update, context):
    ...
    if tool == "image_generation":
        ...
        try:
            ...
            image = await openai_image_generate(
                text
            )
...
async def process_image(
    ...
    if tool == "image_convert":
        ...
        try:
            ...
            await update.effective_message.reply_document(
                document=io.BytesIO(result),
                filename=filename,
                caption="🖼️ تم تحويل الصورة إلى PNG.",
                reply_markup=back_home()
            )
    ...
    if tool in {
        ...
        try:
            ...
            result = await openai_edit_image(
                data,
                prompt
            )
...
async def photo_handler(update, context):
    ...
    try:
        ...
        file = await update.message.photo[
            -1
        ].get_file()
...
async def document_handler(update, context):
    ...
    try:
        ...
        if tool == "pdf_summary":
            ...
            try:
                ...
                if not text.strip():
                    ...
                    raise RuntimeError(
                        "هذا PDF لا يحتوي على نص قابل "
                        "للاستخراج. إذا كان PDF عبارة "
                        "عن صور، نحتاج إضافة OCR للـPDF."
                    )

                result = await openrouter_chat(
                    "لخص هذا المستند بالعربية، "
                    "واذكر أهم النقاط والمعلومات المهمة:\n\n"
                    + text,
                    system=(
        ...
        await update.message.reply_text(
            "❌ نوع الملف غير مناسب لهذه الأداة.",
            reply_markup=back_home()
        )
...
def main():
    ...
    logger.info(
        "AI TOOLS BOT is active and running..."
    )
